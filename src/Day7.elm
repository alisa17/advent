...
parseRulesString : String -> List ( String, List ( Int, String ) )
parseRulesString s =
    s
        |> String.replace "bags" ""
        |> String.replace "bag" ""
        ...
        |> List.map
            (\x ->
                ( List.head (String.split "contain" x) |> Maybe.withDefault ""
                , List.reverse (String.split "contain" x)
                    |> List.head
                    |> Maybe.withDefault ""
                )
            )
        |> List.map
            (\( x, y ) ->
                ...
                , String.replace "." "" y
                    ...
                    |> List.map
                        (\z ->
                            ( List.head (String.split " " z) |> Maybe.withDefault "" |> String.toInt |> Maybe.withDefault 0
                            , String.dropLeft 1 z |> String.trim
                            )
                        )
                )
            )
...
howMany : ( Int, List String, List BagRule ) -> ( Int, List String, List BagRule )
howMany ( i, b, rs ) =
    let
        new =
            List.filter (bagContains b) rs |> List.head
    in
    case new of
        Nothing ->
            ( i, b, rs )
        ...
        Just c ->
            howMany ( i + 1, Tuple.first c :: b, List.filter (\x -> x /= c) rs )


bagContains : List String -> BagRule -> Bool
bagContains c ( p, cs ) =
    List.filter (\x -> List.member x (List.map Tuple.second cs)) c |> (\x -> List.length x > 0)


part1Calc : String -> Int
part1Calc s =
    parseRulesString s
        |> (\x -> howMany ( 0, [ "shiny gold" ], x ))
        |> (\( a, b, c ) -> a)


part2Calc : String -> Int
part2Calc s =
    parseRulesString s
        |> (\x -> howManyInside ( 0, [ ( 1, "shiny gold" ) ], x ))
        |> (\( a, b, c ) -> a)


howManyInside :
    ( Int, List ( Int, String ), List BagRule )
    -> ( Int, List ( Int, String ), List BagRule )
howManyInside ( i, b, rs ) =
    case b of
        [] ->
            ( i, b, rs )
        ...
        _ ->
            let
                newChildren =
                    b
                        |> List.map
                            (\( pc, p ) ->
                                ( pc
                                , List.filter (\y -> Tuple.first y == p) rs
                                    |> List.head
                                    |> Maybe.withDefault ( "", [] )
                                    |> Tuple.second
                                )
                ...
                newChildrenNames =
                    newChildren
                        |> List.map
                            (\( x, y ) -> List.map (\( u, v ) -> ( u * x, v )) y)
                        |> List.foldl (++) []
                        |> List.filter (\( v, w ) -> v /= 0)
            in
            howManyInside ( i + newChildrenCount, newChildrenNames, rs )
